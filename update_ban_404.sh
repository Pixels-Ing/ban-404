...
C_opt_v=( $'PLATFORM_VALIDATED\t#PLATFORM_VALIDATED=""' $'FW_BACKEND\t#FW_BACKEND="auto"' $'LOG_PROFILE\t#LOG_PROFILE="auto"' $'WINDOW\t#WINDOW=7200' $'BAN_TIMEOUT\t#BAN_TIMEOUT=172800' $'TAIL_LINES\t#TAIL_LINES=50000' $'BAN_THRESHOLD\t#BAN_THRESHOLD=10' $'DISTINCT_PATH_MIN\t#DISTINCT_PATH_MIN=2' $'HONEYPOT_SCORE\t#HONEYPOT_SCORE=100' $'HONEYPOT_BAN_TIMEOUT\t#HONEYPOT_BAN_TIMEOUT=604800' $'BAN_ESCALATION\t#BAN_ESCALATION="604800 1209600 2073600"' $'ESCALATION_MEMORY\t#ESCALATION_MEMORY=2073600' $'AGGRESSIVE_SCORE\t#AGGRESSIVE_SCORE=500' $'POST_FLOOD_THRESHOLD\t#POST_FLOOD_THRESHOLD=20' $'RESOLVE_PTR\t#RESOLVE_PTR=false' $'PTR_TIMEOUT\t#PTR_TIMEOUT=2' $'CRON_STEP\t#CRON_STEP=""' )
...
C_notif_v=( $'SERVER_NICKNAME\t#SERVER_NICKNAME=""' $'WEBHOOK_URL\t#WEBHOOK_URL=""' $'WEBHOOK_CHAT_CARD\t#WEBHOOK_CHAT_CARD=true' $'NOTIFY_EMAIL\t#NOTIFY_EMAIL=""' $'NOTIFY_FROM\t#NOTIFY_FROM=""' $'NOTIFY_MIN_BANS\t#NOTIFY_MIN_BANS=1' $'NOTIFY_BANS\t#NOTIFY_BANS=false' $'NOTIFY_BANS_EMAIL\t#NOTIFY_BANS_EMAIL=""   # destinataire(s) des alertes de ban ; vide = NOTIFY_EMAIL' $'NOTIFY_BANS_CHANNELS\t#NOTIFY_BANS_CHANNELS="all"   # canaux des alertes de ban : all = webhook + e-mail ; email = e-mail seul (NOTIFY_BANS_EMAIL, à défaut NOTIFY_EMAIL) ; webhook = WEBHOOK_URL seul' $'DAILY_SUMMARY\t#DAILY_SUMMARY=false' )
